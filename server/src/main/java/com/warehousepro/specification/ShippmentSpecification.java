  ...
  public Specification<Shipment> hasTrackingNumber(String trackingNumber) {
    return (root, query, criteriaBuilder) ->
      StringUtils.hasText(trackingNumber)
        ? criteriaBuilder.like(criteriaBuilder.lower(root.get("trackingNumber")), "%" + trackingNumber.toLowerCase() + "%")
        : criteriaBuilder.conjunction();
  }
  ...
  public Specification<Shipment> hasShippingMethod(String shippingMethod) {
    return (root, query, criteriaBuilder) ->
      StringUtils.hasText(shippingMethod)
        ? criteriaBuilder.like(criteriaBuilder.lower(root.get("shippingMethod")), "%" + shippingMethod.toLowerCase() + "%")
        : criteriaBuilder.conjunction();
  }
  ...
  public Specification<Shipment> hasCarrier(String carrier) {
    return (root, query, criteriaBuilder) ->
      StringUtils.hasText(carrier)
        ? criteriaBuilder.like(criteriaBuilder.lower(root.get("carrier")), "%" + carrier.toLowerCase() + "%")
        : criteriaBuilder.conjunction();
  }
  ...
  public Specification<Shipment> getFilterSpecification(ListShipmentRequest filterRequest) {
    return (root, query, criteriaBuilder) -> {
      ...
      if (filterRequest.getShipmentDate() != null) {
        predicates.add(criteriaBuilder.equal(root.get("shipmentDate"), filterRequest.getShipmentDate()));
      }
      ...
      if (filterRequest.getDeliveryEstimate() != null) {
        predicates.add(criteriaBuilder.equal(root.get("deliveryEstimate"), filterRequest.getDeliveryEstimate()));
      }
      ...
      if (StringUtils.hasText(filterRequest.getQuery())) {
        ...
        Predicate trackingNumberPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("trackingNumber")), queryParam);
        Predicate carrierPredicate = criteriaBuilder.like(criteriaBuilder.lower(root.get("carrier")), queryParam);
        predicates.add(criteriaBuilder.or(statusPredicate, trackingNumberPredicate, carrierPredicate));
      }
